...
def extract_dt_from_name(fname: str) -> Optional[dt.date]:
    """
    Извлекает дату из имени файла и возвращает dt.date, например 2025-11-19.
    """
    m = re.search(r'(\d{1,2}\.\d{1,2}\.\d{4} \d{1,2}\.\d{1,2})', fname)
    if not m:
        return None
    dt_obj = dt.datetime.strptime(m.group(1), '%d.%m.%Y %H.%M')
    return dt_obj.date()

def last_date_indicator(path_to_file, sep: str = "|"):
    df = pd.read_csv(path_to_file, sep=sep)
    file_name = Path(path_to_file).name
    cutoff_date = extract_dt_from_name(file_name)  # тип: datetime.date, вывод: 2025-11-19
    ...
    if cutoff_date is None:
        last = df['date'].max()
        if pd.isna(last):
            raise ValueError("Не удалось определить последнюю дату в файле чеков.")
        return last.strftime('%Y-%m-%d')

    df = df[df['date'].dt.date < cutoff_date]
    df = df.sort_values(by='date')
    if df.empty:
        return pd.Timestamp(cutoff_date).strftime('%Y-%m-%d')
    LAST_DATE = df['date'].unique()[-1].strftime('%Y-%m-%d')
    return LAST_DATE
